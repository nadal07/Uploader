...
function createUploaderRouter(options = {}) {
  ...
  router.put('/auth/password', authenticate, (req, res) => {
    try {
      ...
      const updatedUser = authService.changeOwnPassword(req.user.id, newPassword);
      return res.json({ success: true, message: 'Password updated successfully', user: updatedUser });
    } catch (err) {
      return res.status(400).json({ error: err.message });
    }
  });

  router.post('/auth/skip-password-change', authenticate, (req, res) => {
    try {
      const updatedUser = authService.skipPasswordChange(req.user.id);
      return res.json({ success: true, message: 'Password change skipped', user: updatedUser });
    } catch (err) {
  ...
  router.put('/admin/users/:userId', authenticate, requireAdmin, (req, res) => {
    try {
      ...
      const { role, allowedFolderIds, password, mustChangePassword } = req.body;
      const updated = authService.updateUser(userId, { role, allowedFolderIds, password, mustChangePassword });
