 ...
 * with Authentication & Target Folder selection support.
 */
class UploaderClient {
   ...
   * @param {string} [options.authToken=null] JWT Authentication Token
   * @param {string} [options.folderId=null] Target folder ID for upload
   */
  constructor(options = {}) {
   ...
   */
  async start(file, overrideOptions = {}) {
    if (!file) throw new Error('File object is required');
    if (overrideOptions.folderId) this.folderId = overrideOptions.folderId;
    if (overrideOptions.authToken) this.authToken = overrideOptions.authToken;

    if (!this.folderId) {
      const err = new Error('Target folder must be selected before starting upload.');
    ...
    try {
      // Step 1: Initialize session on server with target folderId
      ...
      if (!initResponse.ok) {
        const errorJson = await initResponse.json().catch(() => ({}));
        const msg = errorJson.error || `Init failed with status ${initResponse.status}`;
        throw new Error(msg);
      }
  ...
  async completeUpload() {
    try {
      ...
      if (!response.ok) {
        const errJson = await response.json().catch(() => ({}));
        throw new Error(errJson.error || `Complete failed (${response.status})`);
      }
  ...
  notifyProgress() {
    ...
    const speed = elapsedSec > 0 ? this.bytesUploadedSinceStart / elapsedSec : 0;
  ...
  saveLocalState() {
    ...
    try {
      const stateKey = `uploader_session_${this.folderId}_${this.file.name}_${this.file.size}`;
